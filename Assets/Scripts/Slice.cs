...
public class Slice : MonoBehaviour {
    ...
    void OnTriggerEnter2D(Collider2D other) {
        ...
        if (other.CompareTag("Item")) {
            ...
            //Play Slice/Slash sound with randomized Pitch
            sliceSound.pitch = Random.Range(1,3f);
            sliceSound.Play();

            //Play the Slice Animation
            Vector2 lastItemPos = other.transform.position;
            sliceAnimation.transform.position = lastItemPos;
            sliceAnimation.Play();
